...
//this needs to search (loop) through the titles to see if a book is there in NF
//below are two book objects and need to look at the titles specifically
    it("should tell us if a title is on a different shelf", function() {
      ...
      //this array below has w/in it the two titles the patron wants
      var nonFictionShelf = [bossypants, whyNotMe];
// the search needs to return if a book is there
// so the response needs to be checking with a boolean condiiton since
//the return needs to tell if a book is available or not (t/f)
